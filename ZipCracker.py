...
#Coded By Asaotomo               Update:2023.09.25
    ...
    if is_zip_encrypted(zip_file):
        ...
        try:
            ...
            is_numeric = input("密码是否仅包含数字？(是1/否0): ").lower() == "1"
            is_lowercase = input("密码是否包含小写字母？(是1/否0): ").lower() == "1"
            is_uppercase = input("密码是否包含大写字母？(是1/否0): ").lower() == "1"
            is_special = input("密码是否包含特殊字符？(是1/否0): ").lower() == "1"
            characters = ''
            ...
            password_list = [''.join(p) for p in product(characters, repeat=length)]
            print(
                f'[+] 加载{length}位{"数字" if is_numeric else "数字和字母"}{"和小写字母" if is_lowercase else ""}{"和特殊字符" if is_special else ""}{"和大写字母" if is_uppercase else ""}字典成功！')
